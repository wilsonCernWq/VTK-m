...
void ANARIMapperVolume::ConstructArrays(bool regenerate)
{
  ...
  if (!this->Handles->SpatialField) 
  {
    this->Handles->SpatialField = anari_cpp::newObject<anari_cpp::SpatialField>(this->GetDevice(), 
      (isStructured && isScalar) ? "structuredRegular" : "unstructured"
    );
  }
  ...
  // Unstructured volume data
    ...
    if(cells.IsType<vtkm::cont::CellSetSingleType<>>())
    {
      ...
      auto shapes = sgl.GetShapesArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
      vtkm::cont::ArrayCopyDevice(
        vtkm::cont::make_ArrayHandleTransform(shapes, ToAnariCellType{}), 
        arrays.CellType
      );

      // 2. Cell Connectivity
      auto conn = sgl.GetConnectivityArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
      vtkm::cont::ArrayCopyDevice(conn, arrays.Index);

      // 3. Cell Index
      auto offsets = sgl.GetOffsetsArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
    ...
    else if(cells.IsType<vtkm::cont::CellSetExplicit<>>())
    {
      ...
      auto shapes = exp.GetShapesArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
      vtkm::cont::ArrayCopyDevice(
        vtkm::cont::make_ArrayHandleTransform(shapes, ToAnariCellType{}), 
      ...
      // 2. Cell Connectivity
      auto conn = exp.GetConnectivityArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
      vtkm::cont::ArrayCopyDevice(conn, arrays.Index);

      // 3. Cell Index
      auto offsets = exp.GetOffsetsArray(
        vtkm::TopologyElementTagCell(), vtkm::TopologyElementTagPoint()
      );
      vtkm::cont::ArrayCopyDevice(offsets, arrays.CellIndex);
    }
    ...
    // "vertex.position"
    {
      auto* ptr = (vtkm::Vec3f_32*)arrays.VertexPosition.GetBuffers()[0].ReadPointerHost(*arrays.Token);
      this->Handles->UnstructuredParameters.VertexPosition =
        anari_cpp::newArray1D(d, ptr, NoopANARIDeleter, nullptr, arrays.VertexPosition.GetNumberOfValues());
    }
    ...
    // "vertex.data"
    {
      ...
      this->Handles->UnstructuredParameters.VertexData =
        anari_cpp::newArray1D(d, ptr, NoopANARIDeleter, nullptr, arrays.VertexData.GetNumberOfValues());
    }
    ...
    // "cell.index"
    {
      ...
      this->Handles->UnstructuredParameters.CellIndex =
        anari_cpp::newArray1D(d, ptr, NoopANARIDeleter, nullptr, arrays.CellIndex.GetNumberOfValues()-1);
    }
    ...
    // "cell.type"
    {
      ...
      this->Handles->UnstructuredParameters.CellType =
        anari_cpp::newArray1D(d, ptr, NoopANARIDeleter, nullptr, arrays.CellType.GetNumberOfValues());
    }
...
void ANARIMapperVolume::UpdateSpatialField()
{
  ...
  if (this->Handles->StructuredParameters.Data)
  {
    ...
    anari_cpp::setParameter(d, this->Handles->SpatialField, "data", this->Handles->StructuredParameters.Data);
  }

  if (this->Handles->UnstructuredParameters.VertexPosition)
  {
    anari_cpp::setParameter(
      d, this->Handles->SpatialField, "vertex.position", this->Handles->UnstructuredParameters.VertexPosition);
  }
  if (this->Handles->UnstructuredParameters.VertexData)
  {
    anari_cpp::setParameter(
      d, this->Handles->SpatialField, "vertex.data", this->Handles->UnstructuredParameters.VertexData);
  }
  ...
  {
    anari_cpp::setParameter(
      d, this->Handles->SpatialField, "indexPrefixed", this->Handles->UnstructuredParameters.IndexPrefixed);
  }
  if (this->Handles->UnstructuredParameters.CellIndex) 
  {
    anari_cpp::setParameter(
      d, this->Handles->SpatialField, "cell.index", this->Handles->UnstructuredParameters.CellIndex);
  }
